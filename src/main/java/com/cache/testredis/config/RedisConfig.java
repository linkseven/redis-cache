...
public class RedisConfig {
    /**
     * @param factory：配置不同的客户端，这里注入的redis连接工厂不同： JedisConnectionFactory、LettuceConnectionFactory
    ...
    public RedisTemplate<String,Object> redisTemplate(RedisConnectionFactory factory){
        GenericJackson2JsonRedisSerializer gj2jrs = serializer();
    ...
    public CacheManager cacheManager(RedisConnectionFactory redisConnectionFactory) {
        ...
        for (Map.Entry<String, Long> entry : entries) {
            //指定特定缓存空间对应的过期时间
            map.put("accounts", redisCacheConfiguration.entryTtl(Duration.ofSeconds(40)));
